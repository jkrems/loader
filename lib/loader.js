...
function initByMime(url, wrap, sourceText) {
  ...
  switch (mimeType) {
    case 'text/javascript': {
      ...
      // HAAACK - this internal structure is super confusing
      const urlObject = new URL(url);
      if (urlObject.protocol === 'node-core:') {
        wrap.instantiate();
        const hole = wrap.evaluate();
        typeof hole === 'function' && hole(require, urlObject.pathname);
      }
...
function createCommonJSWrap(urlObject) {
  return {
    content: Buffer.from(`\
let $default;
export { $default as default };

(function hole(loader, pathname) {
  const values = loader(pathname);
  $default = values;
});
`),
  ...
  constructor() {
    ...
    this._moduleMap = new ModuleMap(this, {
      async get(url) {
        ...
        const urlObject = new URL(url);
        if (urlObject.protocol === 'node-core:') {
          return createCommonJSWrap(urlObject);
        } else if (urlObject.protocol !== 'file:') {
          throw new TypeError(
            `Only file: protocol is supported, got ${urlObject}`
          );
        }
...
function registerUnprefixedNodeCoreModules() {
  ...
  for (const builtin of builtinModules) {
    loader['_packageMap'].set(`import:${builtin}`, `node-core:${builtin}`);
  }
