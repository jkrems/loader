...
function initByMime(url, wrap, sourceText) {
  ...
  switch (mimeType) {
    ...
    case 'text/vnd.node.js': {
      ...
      wrap.setLazyStaticExports(exportKeys, () => {
    ...
    case 'application/wasm': {
      ...
      /* global WebAssembly */
      const wasmModule = new WebAssembly.Module(sourceText.content);
      ...
      const wasmImports = importArr
        .map(({ module: specifier, name }, idx) => {
          ...
          if (specifier === '') {
            throw new TypeError(`Use of invalid specifier "" in ${url}`);
          }
...
`;
      ...
      const moduleProvider = new Module(`wasm-compile:${url}`);
...
(function setModule(m) {
  module = m;
      ...
      wrap.compile(wasmWrap);
      wrap.resolveRequest('', moduleProvider);
      break;
    }
