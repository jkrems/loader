...
function getMimeHandler(url) {
  const extIdx = url.lastIndexOf('.');
  const extension = url.slice(extIdx + 1);
  switch (extension) {
    case 'mjs':
      return function compileJS(wrap, sourceText) {
        wrap.compile(sourceText.toString());
      };

    case 'wasm':
      // super simplistic and just for demo purposes.
      // the way this is set up doesn't support dependencies etc.
      return function compileWASM(wrap, sourceText) {
        /* global WebAssembly */
...
(function initMirrors($_data_) {
  ${inits}
    ...
    default:
      throw new TypeError(`Unsupported url extension ${extension} for ${url}`);
  }
}
...
class ResolveModuleJob {
  ...
  async initialize(url, wrap) {
    ...
    const mimeHandler = getMimeHandler(url);
    mimeHandler(wrap, sourceText);
  ...
  constructor() {
    ...
    this._moduleMap = new ModuleMap(this, {
      async get(url) {
        ...
        const sourceText = await readFile(fileURL);
        return sourceText;
      },
    });
  }
